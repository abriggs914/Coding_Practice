...
/****** Object:  View [dbo].[v_TeamsInfo]    Script Date: 2022-06-20 11:56:05 PM ******/
SET ANSI_NULLS ON
GO
...
--AS
	...
	SELECT
		...
		, [TopAffiliation] -- Top Affliation based on numbers
		, [TopAffiliationNum] -- Top Affliation number
	FROM (
		SELECT 
			[Teams].[ID] AS [TeamID]
			...
			, 0 AS [TopAffiliation] -- Top Affliation based on numbers
			, 0 AS [TopAffiliationNum] -- Top Affliation number
		FROM
			[Teams]
		INNER JOIN
			[Games]
		...
		SELECT
			...
			, [APF] -- Average Points For
			, [APA] -- Average Points Against
			
			, [PFGW] -- Points For Game Win
			, [PAGW] -- Points Against Game Win
			, [PDGW] -- Points Difference Game Win
			, [APFGW] -- Average Points For Game Win
			, [APAGW] -- Average Points Against Game Win
			
			, [PFGL] -- Points For Game Loss
			, [PAGL] -- Points Against Game Loss
			, [PDGL] -- Points Difference Game Loss
			, [APFGL] -- Average Points For Game Loss
			, [APAGL] -- Average Points Against Game Loss
		...
		FROM (
			SELECT 
				...
				, 0 AS [NumPlayers] -- Number Players
				, [Affiliation] AS [TopAffiliation]
				, COUNT(*) AS [TopAffiliationNum]
				, ROW_NUMBER() OVER(
					PARTITION BY
						[Affiliation]
					ORDER BY
