...
def page_hockey_pool():
    ...
    radio_hp_mode = st.radio(
        "Mode",
        ["Stats", "Predictions", "Pools", "PickSheets"],
        key=k_radio_hp_mode,
        horizontal=True
    )
    ...
    if radio_hp_mode == "Picksheets":
        ...
        if not df_pool_year.empty:
            ...
            display_df_nhl(df_game_scoring, "YUYUY")
        ...
        with st.container(border=True):
            ...
            df_all_picks = pd.concat(df_all_picks, ignore_index=True)
                
            display_df(df_all_picks, "df_all_picks")
            
            ###
            ###
            df_selections = df_all_picks[df_all_picks["val"] == 1]
            st.divider()
            st.subheader("HERE")
            df_pick_sim = []
            for i, row in df_picksheets.iterrows():
            ...
            cols = df_pick_sim.columns.tolist()[2:]
            cols.sort()
            df_pick_sim = df_pick_sim[["year", "player"] + cols]
            
            df_pick_sim = df_pick_sim.merge(df_people, left_on="player", right_on="id", how="inner")
            df_pick_sim = df_pick_sim[["year", "name"] + cols]
            ...
            display_df(df_pick_sim, f"df_pick_sim", fail_safe=False)
            ...
            display_df(df_all_picks_g, "df_all_picks_g")
            ...
            df_c["team"] = df_c["team"].apply(lambda t: fetch_team_logo(t, False, True))
            st.write(f"COMBINED")
            display_df(
                df_c, "comb",
                column_config={"team": st.column_config.ImageColumn(label="team", width=40)}
            )
            # with st.container(horizontal=True):
            #     for v in ["LAK", "LA", "SEA"]:
            #         for b in ["Light", "Dark"]:
...
def display_df_nhl(
    ...
    team_cols = ["", "away", "home"]
    team_cols += [f"{t}team" for t in team_cols]
    ...
    i_cols = []
    for col_t, col in df_cols.items():
    ...
    for c in i_cols:
        ...
        column_config[c] = st.column_config.ImageColumn(c)
