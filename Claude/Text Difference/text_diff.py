...
def char_diff_html(line_a, line_b, mode):
    """
    Returns HTML with inline char-level highlights for a pair of changed lines.
    mode: 'removed' or 'added'
    """
    matcher = difflib.SequenceMatcher(None, line_a, line_b, autojunk=False)
    result = []
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if mode == "removed":
            chunk = html_module.escape(line_a[i1:i2])
            if tag == "equal":
                result.append(f'<span class="diff-text-removed">{chunk}</span>')
            elif tag in ("replace", "delete"):
                result.append(f'<span class="char-removed">{chunk}</span>')
        else:  # added
            chunk = html_module.escape(line_b[j1:j2])
            if tag == "equal":
                result.append(f'<span class="diff-text-added">{chunk}</span>')
            elif tag in ("replace", "insert"):
                result.append(f'<span class="char-added">{chunk}</span>')
    return "".join(result) if result else "&nbsp;"


# Header
st.markdown("<h1>Text Diff</h1>", unsafe_allow_html=True)
st.markdown('<p class="subtitle">Compare two text snippets — line by line, character by character</p>', unsafe_allow_html=True)
            ...
            if tag == "equal":
                for line in lines_a[i1:i2]:
                    ...
                    diff_lines_html.append(
                        ...
                        f'<span class="diff-text-unchanged">{_h.escape(line) if line else "&nbsp;"}</span>'
                        f'</div>'
                    )
            elif tag in ("replace", "delete"):
                chunk_a = lines_a[i1:i2]
                chunk_b = lines_b[j1:j2] if tag == "replace" else []
                # Pair up lines for char-level diff where possible
                for idx, line in enumerate(chunk_a):
                    removed_count += 1
                    paired = chunk_b[idx] if idx < len(chunk_b) else None
                    if paired is not None:
                        inner = char_diff_html(line, paired, "removed")
                    else:
                        import html as _h
                        inner = f'<span class="diff-text-removed">{_h.escape(line) if line else "&nbsp;"}</span>'
                    diff_lines_html.append(
                ...
                if tag == "replace":
                    for idx, line in enumerate(chunk_b):
                        added_count += 1
                        paired = chunk_a[idx] if idx < len(chunk_a) else None
                        if paired is not None:
                            inner = char_diff_html(paired, line, "added")
                        else:
                            import html as _h
                            inner = f'<span class="diff-text-added">{_h.escape(line) if line else "&nbsp;"}</span>'
                        diff_lines_html.append(
                ...
                for line in lines_b[j1:j2]:
                    ...
                    diff_lines_html.append(
                        f'<div class="diff-line diff-line-added">'
                        f'<span class="diff-marker diff-marker-added">+</span>'
                        f'<span class="diff-text-added">{_h.escape(line) if line else "&nbsp;"}</span>'
                        f'</div>'
                    )
