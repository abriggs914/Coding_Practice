...
public class Deck{
	...
	Card[] originalDeckIndex = new Card[52];
	...
	public Deck(){    //creats a deck object of 52 cards in ascending order in the suit order above.
		...
		for(int k  = 0; k < 52; k++){
			...
			if(i < 11 && i > 1){
				workingCard = new Card(k, i, currSuit, k);
				...
				char mode = 'y';
				workingCard = new Card(k, faceValue, currSuit, mode, k);
				//System.out.println("New Card: " + workingCard.toString() + ", Index: " + workingCard.index);
				i = holder;
			}
	...
	public Card drawNewCards(Deck myDeck, int number){
	...
	public boolean checkIfAvailable(int num, int[] arr, Deck deckIn){
	...
	public Card getCardFromId(Deck deckIn, int num){
		int i = 0;
		for(i = 0; i < 52; i++){
	...
	public void updateDeck(Hand handIn){	
		System.out.println("\n\n\tUpdating Deck\n\n");
		int intTarget = 0;
		String stringTarget = "";
