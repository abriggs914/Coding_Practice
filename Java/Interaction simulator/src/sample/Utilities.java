...
public class Utilities {
    ...
    private static final int TABLE_CERTAINTY_LENGTH = 10000;
    private static int[] ageTable;
    private static int[] expectancyTable;
    private static double[][] survivalTable;
    ...
    public static int getRandomAge(double averageAge) {
        if (ageTable == null) {
    ...
    public static double genRandomLifeSpan(double age, double avgLifeExpectancy) {
        if (expectancyTable == null) {
            expectancyTable = getRandomList((int) Math.round(avgLifeExpectancy), TABLE_CERTAINTY_LENGTH);
        }
        int lifeExpectancy = 365 * expectancyTable[(int) randomDoubleInRange(0, expectancyTable.length)];
    ...
    private static void genSurvivalTable() {
        ...
        // [age, P(D|M), N(L|M), E(L|M), P(D|F), N(L|F), E(L|F)]
        survivalTable = new double[][] {
    ...
    public static String formatify(int i) {
        NumberFormat nf = NumberFormat.getInstance();
        nf.setMaximumFractionDigits(0);
        nf.setMaximumFractionDigits(0);
        nf.setMaximumIntegerDigits(5);
        nf.setMinimumIntegerDigits(5);
        return nf.format(i).replaceAll(",", "").trim();
    }
}
