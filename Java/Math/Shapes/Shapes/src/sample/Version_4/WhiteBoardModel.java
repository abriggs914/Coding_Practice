package sample.Version_4;

import javafx.collections.ObservableList;
import javafx.geometry.Point2D;
import javafx.scene.Node;
import javafx.scene.Parent;
import javafx.scene.layout.Pane;
import javafx.scene.paint.Color;
import javafx.scene.paint.Paint;
import javafx.scene.shape.Line;
import javafx.scene.shape.Polygon;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.HashMap;
import java.util.List;

public class WhiteBoardModel {



    // Default design and style variables
    public static boolean check_drawGrid = true;
    public static int CELL_WIDTH;
    public static int CELL_HEIGHT;
    public static Color GRID_LINE_COLOUR;
    public static boolean check_drawIntersectionsInterior = false;
    public static boolean check_drawIntersectionsSupplementA = false;
    public static boolean check_drawIntersectionsSupplementB = false;
    public static boolean check_drawIntersectionsOpposite = false;

    private static Parent root;
    private static Paint BACKGROUND;
    private static Point2D markerPointFirst;
    private static Point2D markerPointLast;
    private static Line drawingLine;
    private static Shape adjustingShape;

    private static double whiteBoardMarkerRedVal;
    private static double whiteBoardMarkerGreenVal;
    private static double whiteBoardMarkerBlueVal;
    private static double[] whiteBoardMarkerColour;
    private static double opacity = 1;

//    private static ArrayList<Line> whiteBoardLines;
    private static ArrayList<Triangle> whiteBoardTriangles;
    private static int triangleNumber;

    public WhiteBoardModel() {

        CELL_WIDTH = 10;
        CELL_HEIGHT = 10;

        GRID_LINE_COLOUR = Color.color(0.55, 0.55, 0.55, 0.7);
        BACKGROUND = Color.color(0.9, 0.65, 0.65, 0.75);//Color.color(1,1,1,1);
        whiteBoardMarkerRedVal = 1;
        whiteBoardMarkerGreenVal = 1;
        whiteBoardMarkerBlueVal = 1;

        updateWhiteBoardMarkerColour();


//        whiteBoardLines = new ArrayList<>();
        whiteBoardTriangles = new ArrayList<>();
        triangleNumber = 0;
    }

    public static void updateWhiteBoardRed(double value) {
        whiteBoardMarkerRedVal = value;
        updateWhiteBoardMarkerColour();
    }

    public static void updateWhiteBoardGreen(double value) {
        whiteBoardMarkerGreenVal = value;
        updateWhiteBoardMarkerColour();
    }

    public static void updateWhiteBoardBlue(double value) {
        whiteBoardMarkerBlueVal = value;
        updateWhiteBoardMarkerColour();
    }

    private static void updateWhiteBoardMarkerColour() {
        whiteBoardMarkerColour = new double[] {
                whiteBoardMarkerRedVal,
                whiteBoardMarkerGreenVal,
                whiteBoardMarkerBlueVal,
                1.0
        };
        updateDrawingLineColour();
    }

    public static void updateDrawingLineColour() {
        if (drawingLine != null) {
            drawingLine.setStroke(getWhiteBoardColour());
            drawingLine.setFill(getWhiteBoardColour());
        }
    }

    public static void addTriangle(Triangle t) {
        Polygon p = t.getPolygon();
        for (Triangle triangle : whiteBoardTriangles) {
            Polygon tp = triangle.getPolygon();
            if (matchingPolygons(p, tp)) {
                return;
            }
        }
////        Triangle triangle = createTriangle(p);
////        whiteBoardTriangles.put(p, triangle);
        whiteBoardTriangles.add(t);
    }

    public static double[] getWhiteBoardMarkerColour() {
        return whiteBoardMarkerColour;
    }

    public static Color getWhiteBoardColour() {
        double[] markerColour = getWhiteBoardMarkerColour();
        double r = markerColour[0];
        double g = markerColour[1];
        double b = markerColour[2];
        double a = markerColour[3];
        return new Color(r, g, b, a);
    }

//    public static void addLine(Line l) {
//        whiteBoardLines.add(l);
//    }

//    public static void addLines(Line... l) {
//        whiteBoardLines.addAll(Arrays.asList(l));
//    }
//
//    public static void removeLines(Line... lines) {
//        int s = whiteBoardLines.size();
//        whiteBoardLines.removeAll(Arrays.asList(lines));
//        int a = whiteBoardLines.size();
//        if (s - lines.length != a) {
//            ArrayList<Line> wbLines = new ArrayList<>();
//            for (Line wbLine : whiteBoardLines) {
//                boolean include = true;
//                for (Line l : lines) {
//                    if (matchingLines(wbLine, l)) {
//                        include = false;
//                        break;
//                    }
//                }
//                if (include) {
//                    wbLines.add(wbLine);
//                }
//            }
//            whiteBoardLines.clear();
//            whiteBoardLines.addAll(wbLines);
//        }
//    }

//    public static ArrayList<Line> getWhiteBoardLines(ObservableList<Node> children) {
//        return new ArrayList<>(getIntersectingWhiteBoardLines(children).keySet());
//    }

//    public static void clearWhiteBoardLines() {
//        whiteBoardLines.clear();
//    }
//
    public static void clearWhiteBoardTriangles() {
        whiteBoardTriangles.clear();
    }

    public static HashMap<Line, ArrayList<Line>> getIntersectingWhiteBoardLines(ObservableList<Node> children) {
        HashMap<Line, ArrayList<Line>> intersections = new HashMap<>();
        for (Node key : children) {
            if (key.getClass() == Line.class) {
                Line k = (Line) key;
                ArrayList<Line> lines = new ArrayList<>();
                for (Node val : children) {
                    if (val.getClass() == Line.class) {
                        if (!key.equals(val)) {
                            Line v = (Line) val;
                            if (intersectionPoint(k, v) != null) {
                                lines.add(v);
                            }
                        }
                    }
                }
                intersections.put(k, lines);
            }
        }
        return intersections;
    }

//    }public static HashMap<Line, ArrayList<Line>> getIntersectingWhiteBoardLines() {
//        HashMap<Line, ArrayList<Line>> intersections = new HashMap<>();
//        for (Line a : whiteBoardLines) {
//            ArrayList<Line> crosses = new ArrayList<>();
//            for (Line b : whiteBoardLines) {
//                if (a != b) {
//                    if (intersectionPoint(a, b) != null) {
//                        if (intersections.containsKey(b)) {
//                            if (!intersections.get(b).contains(a)) {
//                                crosses.add(b);
////                                System.out.println("if if intersection point: " + WhiteBoardModel.intersectionPoint(a, b));
//                            }
//                        } else {
//                            crosses.add(b);
////                            System.out.println("if else intersection point: " + WhiteBoardModel.intersectionPoint(a, b));
//                        }
//                    }
//                }
//            }
//            if (crosses.size() > 0) {
//                intersections.put(a, crosses);
//            }
//        }
//        return intersections;
//    }

    public static Point2D intersectionPoint(Line a, Line b) {
        double x1a = a.getStartX();
        double y1a = a.getStartY();
        double x2a = a.getEndX();
        double y2a = a.getEndY();

        double x1b = b.getStartX();
        double y1b = b.getStartY();
        double x2b = b.getEndX();
        double y2b = b.getEndY();

        double xda = x2a - x1a;
        double xdb = x2b - x1b;
//         x difference is 0 for 1 or both lines.
        if (xda == 0 || xdb == 0) {
            return null;
        }
        double ma = (y2a - y1a) / xda;
        double mb = (y2b - y1b) / xdb;

        double ba = y1a - (x1a * ma);
        double bb = y1b - (x1b * mb);

        double x = (bb - ba) / (ma - mb);
        double y = (ma * x) + ba;
//        System.out.println(
//                "x1a: " + x1a +
//                "\ny1a: " + y1a +
//                "\nx2a: " + x2a +
//                "\ny2a: " + y2a +
//                "\nx1b: " + x1b +
//                "\ny1b: " + y1b +
//                "\nx2b: " + x2b +
//                "\ny2b: " + y2b +
//                "\n y = " + ma + "x + " + ba +
//                "\n y = " + mb + "x + " + bb
//        );
//        TODO: sometimes produces NaN
        Point2D p = new Point2D(x, y);
        if (!a.contains(p) || !b.contains(p)) {
            return null;
        }
        return p;
    }

    public static ArrayList<Triangle> getWhiteBoardTriangles() {
        return whiteBoardTriangles;
    }

    public static ArrayList<Triangle> getWhiteBoardTriangles(ObservableList<Node> children) {
//        System.out.println("\n\tChildren");
//        for (Node child: children) {
//            System.out.println("child: " + child);
//        }
        ArrayList<Triangle> triangles = getWhiteBoardTriangles();
        HashMap<Line, ArrayList<Line>> intersections = getIntersectingWhiteBoardLines(children);
//        for (Line l : intersections.keySet()) {
//            System.out.println("\n\tk: " + l + "\t\tlist: " + intersections.get(l));
//        }
        for (Line a : intersections.keySet()) {
            for (Line b : intersections.get(a)){
                if (intersections.containsKey(b)) {
//                    System.out.println("a: " + a + "\nb: " + b);
                    ArrayList<Line> bsToCs = intersections.get(b);
                    for (Line c : bsToCs) {
                        if (intersections.get(a).contains(c)) {
//                            System.out.println("\tc: " + c);
                            Polygon p = Triangle.createPolygon(a, b, c, Color.WHITE);
                            boolean include = true;
                            for (Triangle tri : triangles) {
//                                System.out.println("tri.p: " + tri.getPolygon() + "\np: " + p + "\nmatch: " + matchingPolygons(p, tri.getPolygon()));
                                if (matchingPolygons(p, tri.getPolygon()) || matchingLines(a, b, c)) {
                                    include = false;
                                    break;
                                }
                            }
                            if (include) {
                                System.out.println("triangles: " + whiteBoardTriangles + "\np: " + p);
                                Triangle t = createTriangle(genTriangleName(), a, b, c);
                                triangles.add(t);
                                break;
                            }
                        }
                    }
                }
            }
        }
        return triangles;
    }

    public static boolean matchingPolygons(Polygon a, Polygon b) {
        ObservableList<Double> aPoints = a.getPoints().sorted();
        ObservableList<Double> bPoints = b.getPoints().sorted();
        if (aPoints.size() == bPoints.size() && aPoints.size() == 6) {
            for (int i = 0; i < aPoints.size(); i++) {
//                System.out.println("\taPoints.get(i): " + aPoints.get(i) + "\n\tbPoints.get(i): " + bPoints.get(i) + "\n\taPoints.get(i).equals(bPoints.get(i): " + aPoints.get(i).equals(bPoints.get(i)));
                if (!String.format("%.10f", aPoints.get(i)).equals(String.format("%.10f", bPoints.get(i)))) {
                    return false;
                }
            }
        }
        return true;
    }

    public static boolean matchingLines(Line a, Line b, Line c) {
        for (Triangle t : whiteBoardTriangles) {
            Line ta = t.getSideA().getLine();
            Line tb = t.getSideB().getLine();
            Line tc = t.getSideC().getLine();
            ArrayList<Line> ts = new ArrayList<>(Arrays.asList(ta, tb, tc));
            if (ts.indexOf(a) >= 0 && ts.indexOf(b) >= 0 && ts.indexOf(c) >= 0) {
                return true;
            }
        }
        return false;
    }

    /**
     * Checks if two given lines cover the same region.
     * @param a First line.
     * @param b Second line.
     * @return True if they cover the same region.
     */
    public static boolean matchingLines(Line a, Line b) {
        double aXS = a.getStartX();
        double aYS = a.getStartY();
        double aXE = a.getEndX();
        double aYE = a.getEndY();
        double bXS = b.getStartX();
        double bYS = b.getStartY();
        double bXE = b.getEndX();
        double bYE = b.getEndY();
        return (aXS == bXS && aXE == bXE && aYS == bYS && aYE == bYE) || (aXS == bXE && aXE == bXS && aYS == bYE && aYE == bYS);
    }

    public static Triangle createTriangle(String name, Line a, Line b, Line c) {
        Triangle t = new Triangle(name, a, b, c);
        System.out.println("\n\tNew triangle:\n" + t);
        return t;
    }

    public static AdjustableTriangle createAdjustableTriangle(String name, Line a, Line b, Line c) {
        AdjustableTriangle t = new AdjustableTriangle(name, a, b, c);
        System.out.println("\n\tNew adjustable triangle:\n" + t);
        return t;
    }

//    public static Triangle createTriangle(Polygon triangle) {
//        ObservableList<Double> points = triangle.getPoints();
//        Point2D ab = new Point2D(points.get(0), points.get(1));
//        Point2D ac = new Point2D(points.get(2), points.get(3));
//        Point2D bc = new Point2D(points.get(4), points.get(5));
////        double a = ab.distance(ac);
////        double b = ab.distance(bc);
////        double c = ac.distance(bc);
//        Edge sideA = new Edge("side a", ac.getX(), ac.getY(), ab.getX(), ab.getY());
//        Edge sideB = new Edge("side b", ab.getX(), ab.getY(), bc.getX(), bc.getY());
//        Edge sideC = new Edge("side c", bc.getX(), bc.getY(), ac.getX(), ac.getY());
//        Triangle t = new Triangle(genTriangleName(), sideA, sideB, sideC, null, null, null);
//        t.solveRemainingTriangle();
//        System.out.println("\n\tNew triangle:\n" + t);
////        for (int i = 0; i < points.size(); i+=2) {
////            Double x = points.get(i);
////            Double y = points.get(i + 1);
////            Point2D p = new Point2D(x, y);
////            if (i < 2) {
////                sideA = p.;
////            }
////            else if (i < 4) {
////                sideB = ;
////            }
////            else {
////                sideC = ;
////            }
////        }
//        return t;
//    }

    public static Paint getBackgroundColour() {
        return BACKGROUND;
    }

    public static void setParent(Parent rootIn) {
        root = rootIn;
    }

    public static String genTriangleName() {
        triangleNumber++;
        return "Triangle " + triangleNumber;
    }

    public static void resetTriangleNumber() {
        triangleNumber = 0;
    }

    public static void setPointFirst(Point2D point2D) {
        markerPointFirst = point2D;
        if (drawingLine  == null) {
            initDrawingLine();
        }
        if (point2D != null) {
            drawingLine.setStartX(point2D.getX());
            drawingLine.setStartY(point2D.getY());
        }
        else {
            initDrawingLine();
        }
    }

    public static void setPointLast(Point2D point2D) {
        markerPointLast = point2D;
        if (drawingLine  == null) {
            initDrawingLine();
        }
        if (point2D != null) {
            drawingLine.setEndX(point2D.getX());
            drawingLine.setEndY(point2D.getY());
        }
        else {
            initDrawingLine();
        }
    }

    public static Point2D getPointFirst() {
        return markerPointFirst;
    }

    public static Point2D getPointLast() {
        return markerPointLast;
    }

    public static Line getDrawingLine() {
        if (markerPointFirst == null || markerPointLast == null) {
            return null;
        }
        if (drawingLine == null) {
            initDrawingLine();
        }
        return drawingLine;
    }

    public static void initDrawingLine() {
        drawingLine = new Line(0,0,0,0);
        drawingLine.setFill(getWhiteBoardColour());
        drawingLine.setStroke(getWhiteBoardColour());
        drawingLine.setStrokeWidth(5);
    }

    public static Shape getAdjustingShape() {
        return adjustingShape;
    }
}
