...
from resources.resource import PATH_JERSEY_COLLECTION_DATA, PATH_UNKNOWN_IMAGE, JERSEY_COLOUR_SAVE_FILE, DEFAULT_TEAM, UTC_FMT, NHL_URL, \
...
def format_game_header(state: str, result: str, period: int, time_left: str, start_time: str, debug: bool = False) -> tuple[str, str]:
...
start_page = options_pills_mode.index("Dashboard")
...
if pills_mode == options_pills_mode[2]:
    ...
    if pills_standings == options_standings[3]:
        ...
        def get_roots(node, edges_: list):
            n_id = node if isinstance(node, str) else node.id
            n_rnd = int(node[4] if len(n_id) > 4 else n_id[3])
            def get_roots_(node, edges_, n_rnd_=None):
                n_id = node if isinstance(node, str) else node.id
                n_rnd = int(node[4] if len(n_id) > 4 else n_id[3])
                if n_rnd == 0:
                    return [node]
                    ...
                    sources = get_connected_to(node, edges_, sources=True, targets=False)
                    if len(sources) == 2:
                        return get_roots_(sources[0], edges_, n_rnd_=n_rnd_-1) + get_roots_(sources[1], edges_, n_rnd_=n_rnd_-1)
                    elif len(sources) == 1:
                        return get_roots_(sources[0], edges_, n_rnd_=n_rnd_-1)
                    else:
                        return []
            return get_roots_(node, edges_, n_rnd_=n_rnd)

        def get_connected_to(node, edges_: list, sources: bool = True, targets: bool = True):
            n_id = node if isinstance(node, str) else node.id
            connected = []
            for e in edges_:
                if sources and (e.to == n_id):
                    connected.append(e.source)
                if targets and (e.source == n_id):
                    connected.append(e.to)
            return connected
        ...
        seq_pac = df_standings_now_pac.reset_index(drop=True).loc[0, "conferenceSequence"]
        seq_cen = df_standings_now_cen.reset_index(drop=True).loc[0, "conferenceSequence"]
        seq_atl = df_standings_now_atl.reset_index(drop=True).loc[0, "conferenceSequence"]
        seq_met = df_standings_now_met.reset_index(drop=True).loc[0, "conferenceSequence"]
        pac_gets_low_seed: bool = seq_pac <= seq_cen
        atl_gets_low_seed: bool = seq_atl <= seq_met

        st.write(f"{seq_pac=}, {seq_cen=}, {seq_atl=}, {seq_met=}")
        st.write(f"{pac_gets_low_seed=}, {atl_gets_low_seed=}")

        df_w_top = pd.DataFrame(df_standings_to_show["West WC"].iloc[1 if pac_gets_low_seed else 0]).transpose()
        df_w_bot = pd.DataFrame(df_standings_to_show["West WC"].iloc[0 if pac_gets_low_seed else 1]).transpose()
        df_e_top = pd.DataFrame(df_standings_to_show["East WC"].iloc[1 if atl_gets_low_seed else 0]).transpose()
        df_e_bot = pd.DataFrame(df_standings_to_show["East WC"].iloc[0 if atl_gets_low_seed else 1]).transpose()

        st.write(f"df_w_top")
        st.write(str(type(df_w_top)))
        st.write(df_w_top)

        df_w = pd.concat([
            df_w_top,
            df_standings_to_show["Pacific"],
            df_standings_to_show["Central"].sort_values(by="conferenceSequence", ascending=False),
            df_w_bot
        ], ignore_index=True)
        df_w["sortIdx"] = [1, 0, 2, 3, 7, 6, 4, 5]
        df_w.sort_values(by="sortIdx", inplace=True)

        df_e = pd.concat([
            df_e_top,
            df_standings_to_show["Atlantic"],
            df_standings_to_show["Metropolitan"].sort_values(by="conferenceSequence", ascending=False),
            df_e_bot
        ], ignore_index=True)
        df_e["sortIdx"] = [1, 0, 2, 3, 7, 6, 4, 5]
        df_e.sort_values(by="sortIdx", inplace=True)
        ...
        for i, row in df_w.iterrows():
            nodes.append(Node(
                id=f"n_wr0_{i}",
                title=row["teamCommonName"]["default"],
                label=int(row["conferenceSequence"]),
                level=0,
                shape="image",
                color="white",
                image=row[NHLStandings.Abbr.LURL.value]
            ))

        n_wr0_0, n_wr0_1, n_wr0_2, n_wr0_3, n_wr0_4, n_wr0_5, n_wr0_6, n_wr0_7 = nodes

        for i, div in enumerate(["Pac", "Cen"]):
            for j in range(1, 3):
                nodes.append(Node(
                    id=f"n_wr1_{i}_{j}",
                    title=f"West QF {div} {j}",
                    label=f"West QF {div} {j}",
                    level=1,
                    shepe="image",
                    color="white",
                    image=PATH_UNKNOWN_IMAGE
                ))

        n_wr1_0, n_wr1_1, n_wr1_2, n_wr1_3 = nodes[8:]
        edges.append(Edge(
            n_wr0_0.id,
            n_wr1_0.id
        ))
        ...
        for i, div in enumerate(["Atl", "Met"]):
            nodes.append(Node(
                id=f"n_er2_{i}_{j}",
                title=f"East SF {div}",
                label=f"East SF {div}",
        ...
        for i, div in enumerate(["Atl", "Met"]):
            for j in range(1, 3):
                nodes.append(Node(
                    id=f"n_er1_{i}_{j}",
                    title=f"East QF {div} {j}",
                    label=f"East QF {div} {j}",
                    level=7,
                    shepe="image",
                    color="white",
                    image=PATH_UNKNOWN_IMAGE
        ...
        n_er1_0, n_er1_1, n_er1_2, n_er1_3 = nodes[19:]
        edges.append(Edge(
            n_er1_0.id,
            n_er2_0.id
        ))
        ...
        n_er0_0, n_er0_1, n_er0_2, n_er0_3, n_er0_4, n_er0_5, n_er0_6, n_er0_7 = nodes[23:]
        ...
        checkbox_hier = st.checkbox(
            label="Hierarchy",
            key="checkbox_hier",
            value=True
        )

        checkbox_physics = st.checkbox(
            label="Physics",
            key="checkbox_physics",
            value=False
        )

        config = Config(
            hierarchical=checkbox_hier,
            physics=checkbox_physics,
            width=1200,
            height=675,
            direction="LR"
        )

        cols_playoff_tree = st.columns([0.8, 0.2], border=True)
        graph = st.session_state.get("graph")
        with cols_playoff_tree[1]:
            if graph:
                st.write(graph)
                with st.popover("nodes"):
                    st.write(str(type(nodes[0])))
                    st.write(nodes[0])
                    st.write(nodes[0].__dict__)
                with st.popover("edges"):
                    st.write(str(type(edges[0])))
                    st.write(edges[0])
                    st.write(edges[0].__dict__)

            c_nodes = get_connected_to(graph, edges)
            roots = get_roots(graph, edges)
            clicked_round = int(graph[4]) if len(graph) > 4 else graph[3]
            image = [getattr(n, "image", "?") for n in nodes if getattr(n, "id") == graph]
            st.write(f"{clicked_round=}")
            st.write(f"{image=}")
            st.write(c_nodes)
            st.write(roots)

            def update_winner(n, to_node):
                target = get_connected_to(n, edges, sources=False, target=True)
                if target:
                    while target and (target[0] != to_node):
                        target.image = getattr(n, "image")
                        target = get_connected_to(target, edges, sources=False, target=True)


            nodes_roots = [n for n in nodes if getattr(n, "id") in roots]
            for i, n in enumerate(nodes_roots):
                image = getattr(n, "image")
                lbl = getattr(n, "label")
                btn = st.button(
                    label=lbl,
                    key=f"btn_set_winner_{i}",
                    onclick=lambda n_=n, to_node_=graph: update_winner(n_, to_node_)
                )

        with cols_playoff_tree[0]:
            graph = agraph(
                nodes=nodes,
                edges=edges,
                config=config
            )
            st.session_state.update({"graph": graph})
    ...
    if selectbox_team_calendar:
        ...
        ser_sel_team: pd.Series = df_sel_team.loc[df_sel_team["fullName"].str.lower() == selectbox_team_calendar.lower()].iloc[0]
...
elif pills_mode == options_pills_mode[2]:
    # Investigate Game
    pass
    ...
    df_nhl_jerseys_w["PriceCalc"] = df_nhl_jerseys_w["PriceCalc"].fillna(0 if not checkbox_use_avg_instead_actual else avg_spent_per_jersey_est)
    ...
    df_jerseys_stats["PriceCalc"] = df_jerseys_stats["PriceCalc"].fillna(0 if not checkbox_use_avg_instead_actual else avg_spent_per_jersey_est)
