...
class Grid:
    ...
    def x_y_at_tile(self, t_tile):
        """
        Calculate the top-left (x, y) coordinates of a given tile
        :param t_tile: Tile object
        :return: (x, y) or None, None as coordinates
        """
        for r, tile_row in enumerate(self.tiles):
            for c, tile in enumerate(tile_row):
                if tile == t_tile:
    ...
    def r_c_at_tile(self, t_tile):
        """
        Calculate the r and column of a given tile.
        :param t_tile: Tile object. Should exist in the self.tiles list
        :return: (r, c) or None, None as row and column
        """
        for r, tile_row in enumerate(self.tiles):
            for c, tile in enumerate(tile_row):
                if tile == t_tile:
    ...
    def r_c_at_x_y(self, x, y, bind=False):
        """
        Calculate row and column of the tile that covers (x, y).
        If bind, and (x, y) do not fall on the grid, than the closest tile is used.
        :param x: Coordinate x
        :param y: Coordinate y
        :param bind: Whether to bind (x, y) to the grid space or not.
        :return: (r, c) indices or None, None
        """
        print("({}, {})".format(x, y))
        bound = None, None
    ...
    def x_y_at_r_c(self, r, c):
        """
        Return the top-left (x, y) coordinates of the tile located at r, c.
        None, None if r, c out of range.
        :param r: Row index
        :param c: Col index
        :return: (x, y) as coordinates or None
        """
        if r in range(len(self.tiles)):
            if c in range(len(self.tiles[r])):
