...
#did a list comprehension instead of for loop
daily_transactions_split = [transaction.split(new_delim) for transaction in daily_transactions]
...
transactions_clean = [data.strip() for transaction in daily_transactions_split for data in transaction]
...
thread_sold_split = [colour for thread in thread_sold for colour in thread.split('&')]
...
for colour in thread_colors:
	print('Thread Shed sold {num} threads of {colour} thread today.'.format(num = color_count(colour), colour = colour))
