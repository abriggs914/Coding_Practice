...
def layout_text_flat(
    ...
    OpenSCAD's native text() call does not expose a character-spacing /
    tracking parameter. When spacing is left at 1.0, keep the native text()
    output so the font's real kerning is preserved. When the user changes the
    spacing slider, render the word character-by-character and position each
    glyph manually. This makes the spacing control visible in flat mode too.
    """
    if abs(spacing - 1.0) < 0.001:
        ...
        for ch, width in zip(chars, widths):
            travelled += width / 2
            if ch != " ":
                letter = extruded_text(ch, font_name, size, thickness, "center", valign, bold_amount, italic_degrees)
                pieces.append(translate([x0 + travelled, 0, 0])(letter))
            travelled += width / 2

        model = union()(*pieces) if pieces else union()()
