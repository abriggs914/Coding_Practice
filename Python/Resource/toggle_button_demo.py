...
from tkinter_utility import combo_factory, round_polygon, rounded_rect
...
def test_form():
    ...
    questions = {k.lower().replace("(", "").replace(")", "") : dict(zip(["text", "follow_up", "var"], [k, f, v])) for k, f, v in questions}
    ...
    for q_title, q_data in questions.items():
        ...
        tb, label_data, frame_canvas, btn_data = ToggleButton(frame_toggle_buttons, label_text=q_text).get_objects()
