...
#######################################################################################################################
#######################################################################################################################
from colour_utility import iscolour
...
def list_to_html(items: Union[List[str], List[Tuple[str, str]]], title: str = None, is_ordered: bool = True, style: dict = None, inject_style: bool = False) -> Tuple[str, str]:
...
def test_list_to_html():
    ...
    html_output = list_to_html(items, title='Fruits')
    print(html_output)

    # Example usage with tuple input and no title
    items = ('Monday', 'Tuesday', 'Wednesday')
    html_output, style_output = list_to_html(items, is_ordered=True)
    print(html_output)
    ...
    html_str, style_str = list_to_html(range(-2, 6), is_ordered=True, title="Here is my title!", style=style,
                                       inject_style=False)
    print(f"{html_str=}")
    print(f"{style_str=}")
...
def my_attempt(
    ...
    if background is not None:
        assert _ltds_y(background), "Error, invalid type for param 'font'."
