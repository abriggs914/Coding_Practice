...
if __name__ == "__main__":
    ...
    chart_rect = game.Rect(margin, margin + top_offset, (w - (2 * margin)) / 1.35, h - (2 * margin) - top_offset)
    ...
    def draw_chart():
        ...
        if not logbook_3.transactions_list:
            write_text(game, display, game.Rect((w - (w / 2)) / 2, (h - (h / 2)) / 2, w / 2, h / 2), "No Transactions to Report.", game.font.SysFont("Courier", 25), GRAY_16, WILDERNESS_MINT)
            return
        ...
        if CHART_VIEW_MODE == SUM_OF_MONEY_HANDLED:
            ...
            for i, ent in enumerate(logbook_3.entities_list):
                if ent.id_num != e_pot.id_num:
                    col_rect_spent = game.Rect(
                        chart_rect.x + (c * (entity_col_w + entity_col_offset)) + (entity_col_offset / (0.2)),
                        chart_rect.y + top_chart_offset, entity_col_w,
                        chart_rect.h - (bottom_chart_offset + top_chart_offset + title_height))
                    col_rect_spent.x += entity_col_offset  # not quite
                    col_rects.append((i, col_rect_spent))
        ...
        if col_rects:
            if include_negatives:
                ...
                for i in range(0, ceil(largest_money + 100), 100):
                    ...
                    i += smallest_money
                    drawables.insert(0, (write_text, (game, display, tick_rect, money(i), game.font.SysFont("Arial", 12))))
