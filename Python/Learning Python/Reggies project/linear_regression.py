...
for m in possible_ms:
    for b in possible_bs:
        datapoint = calculate_all_error(m, b, datapoints)
        if datapoint < smallest_error:
#             print("smallest: " + str(smallest_error) + ", (" + str(m) + ")best_m: " + str(best_m) + ", (" + str(b) + ")best_b: " + str(best_b) + ", datapoint: " + str(datapoint))
            smallest_error = datapoint
            best_m = m
            best_b = b
...
# datapoints gets reassigned, and if you  don't move the old definition down to #35 then the wrong data will be used.